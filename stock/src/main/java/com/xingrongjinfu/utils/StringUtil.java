...
public class StringUtil {
	...
	public static String replace(String src, String from, String to) {
		...
		while (true) {
			...
			if (l == -1)
				break;
			gRtnStr = src.substring(0, l) + to
					+ src.substring(l + from.length());
			l += to.length();
			src = gRtnStr;
		}
	 ...
	 * 格式化对象
	 */
	public static String formatObject(Object obj, String[] displayNames)
			throws Exception {
		String result = "";
	...
	public static String getSafeableString(String string) {
		...
		return StringUtil.replace(string, StringUtil.DY, "").trim();
	}
	...
	public static String getSBCCase(String string) {
		String result = StringUtil.nullToEmpty(string);
		if (nullOrBlank(result)) {
			return result;
		}
	...
	public static String getLikeConditionString(String condition, String column) {
		condition = StringUtil.getSafeableString(condition);
		condition = StringUtil.replace(condition, "，", ",");
	...
	public static String getLikeConditionString(String string) {
		string = StringUtil.getSafeableString(string);
		if (string.length() <= 0) {
			return "'%%'";
		}
		string = "'%" + string + "%'";
...
//				+ getTimeBetweenString("2008-05-08", "2008-25-20", false));
//	}
	...
	public static String getStartTime(String timestart, boolean showTime) {
		if (nullOrBlank(timestart)) {
			...
			return "to_timestamp('" + getSafeableString(timestart)
					+ (showTime ? "" : " 00:00:00")
	...
	public static String getEndTime(String timeend, boolean showTime) {
		if (nullOrBlank(timeend)) {
			...
			return "to_timestamp('" + getSafeableString(timeend)
					+ (showTime ? "" : " 23:59:59")
					+ "','yyyy-mm-dd hh24:Mi:ss')";
		}
	}
	 ...
	 * @return 一个时间范围查询条件
	 */
	public static String getTimeBetweenString(String timestart, String timeend,
			boolean showTime) {
		return " between " + getStartTime(timestart, showTime) + " and "
				+ getEndTime(timeend, showTime);
	}
	...
	public static String getStringByObjArray(Object[] obj, String pix) {
	 ...
	 public static int getVerify() {
	        ...
	        // (max)%(max-min+1) + min
	        int verify = random.nextInt(999999) % (999999 - 100000 + 1) + 100000;
	        return verify;
	    }
	     ...
	     * 
	     * @param phone_num 手机号
	     * @return true手机号格式正确 false格式错误
	     */
	    public static boolean checkPhone(String phone_num) {
