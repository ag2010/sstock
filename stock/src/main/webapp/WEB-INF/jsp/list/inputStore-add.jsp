	...
	<form action="" method="post" class="form form-horizontal" id="form-inputStore-add">
		<div class="row cl">   
			<div class="row col-xs-4 col-sm-4  input_bar"> 
				...
				<div class="row col-xs-7 col-sm-7 ">
					<input type="text" readonly="readonly" class="input-text" value="${inputStoreNo}" placeholder="" id="id" name="id"> 
		...
		<div class="row cl">   
			...
			<div class="row col-xs-4 col-sm-4  input_bar"> 
				...
				<div class="row col-xs-7 col-sm-7 ">  
					 <y:select id="type" name="type" codeGroup="[{'id':'1','value':'普通入库单'},{'id':'0','value':'门店退货入库单'}]" selectedValue=""
						cssClass="select" headerKey="" headerValue="请选择">
					</y:select> 
				</div>
			</div>  
...
<script type="text/javascript"> 
	...
	$(document).ready(function(){
		$.post("${context_root}/list/getPassedProcurementList.action?type=inputStore", function(data){ 
			initSelect(data);
		}); 
		
		$.post("${context_root}/product/getProductTree.action", function(data){ 
...
function submitData(){ 
	...
	var index = parent.layer.load();
	 $.ajax({
		url:"${context_root}/list/addInputStoreInfo.action", 
		type:'post',
		async:true ,
		cache:false ,
		data:{
